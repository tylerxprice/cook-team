...
def read_excel_data():
    with zipfile.ZipFile('data/historical/Sample Only - Meals Prep Signup.xlsx') as z:
        ...
        for sh in sheets:
            ...
            # Row 1: Day of week
            # Row 2: Meal type / Special notes
            # Row 3: Excel serial date
            # Date columns start at col 3 (or col 4 if col 3 is blank/formula)
            date_columns = []
            ...
            # Parse member rows (Row 4 onwards)
            members = []
...
def parse_emails():
    ...
    for path in sorted(glob.glob('data/historical/*.eml')):
        with open(path, 'rb') as f:
            ...
            for line in lines:
                # Date header like "*Sun Aug 9 - Brunch*" or "*Mon Aug 10*" or "Sun Jan 4 - Brunch"
...
excel_months = read_excel_data()
emails = parse_emails()

print("================================================================================")
print("             REAL HISTORICAL DATA SUMMARY & VALIDATION AUDIT                    ")
print("================================================================================")
print(f"Total Spreadsheet Tabs: {len(excel_months)}")
print(f"Total Email Announcement Files: {len(emails)}")

for tab_name, data in excel_months.items():
    active_members = [m for m in data['members'] if any(v != 'UNAVAILABLE' for v in m['availability'].values())]
    special_notes_members = [m for m in data['members'] if m['notes']]
    print(f"\n📁 Tab [{tab_name}]: {len(data['dates'])} meal dates | {len(data['members'])} registered members ({len(active_members)} active signups) | {len(special_notes_members)} special instructions")
    
    # Print sample special instructions for this month
    for m in special_notes_members[:5]:
        print(f"   • {m['name']}: \"{m['notes']}\"")
